    ...
    def handle(self, *args, **options):
        ...
        for user1, user2 in direct_pairs:
            ...
            if not existing:
                ...
                self.stdout.write(f'  [=] Direct room exists: {user1.username} <-> {user2.username}')
        ...
        if not group_room:
            ...
            self.stdout.write('  [=] Group room exists: Tech Team')
        ...
        if not User.objects.filter(is_superuser=True).exists():
            ...
            self.stdout.write('  [+] Created superuser: admin (admin@example.com / AdminPass123!)')
